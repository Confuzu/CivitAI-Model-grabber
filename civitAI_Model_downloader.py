...
parser.add_argument("--verify_hashes", action="store_true", help="Verify downloaded files using available hashes (BLAKE3 > SHA256 priority).")
        ...
        with open(file_path, "rb") as f:            
            if hash_type == "BLAKE3":
                try:
                    ...
                    logger_md.warning("BLAKE3 not available, falling back to SHA256")
...
def get_best_hash_for_verification(hashes_dict):
    ...
    Priority: BLAKE3 > SHA256 (fastest to slowest)
    ...
    # Priority order: BLAKE3 (fastest), SHA256 (slow)
    hash_priority = ["BLAKE3", "SHA256"]
...
def process_username(username, download_type):
    ...
    if args.verify_hashes:
        print("Hash verification enabled - using BLAKE3 for fast verification")
