...
logging.basicConfig(filename='civitai_Model_downloader.log', encoding='utf-8', level=logging.WARNING)
...
def download_file_or_image(url, output_path):
    try:
        ...
        # Handle exception, close progress bar, and retry
        progress_bar.close() # Close the progress bar
        print(f"Error downloading: {url}")
        # Other error handling code...
        time.sleep(retry_delay)
        download_file_or_image(url, output_path)  # Retry the download
...
def download_model_files(item_name, model_version):
    files = model_version.get('files', [])
    images = model_version.get('images', [])
    downloaded = False

    # Sanitize the item_name and create the directory for the item
    item_dir = os.path.join(output_dir, sanitize_name(item_name))
    os.makedirs(item_dir, exist_ok=True)  # Ensure the directory exists
    ...
    for _ in range(max_recursion_depth):
        ...
        for item in items:
            ...
            for version in model_versions:
                future = executor.submit(download_model_files, item_name, version)
...
download_results = [future.result() for future in tqdm(download_futures, desc="Downloading Files", unit="file", leave=False)]
...
downloaded_item_names = {item_name for item_name, _, _ in download_results if _}
missing_items = all_item_names - downloaded_item_names
...
# Print missing item names if any
if missing_items:
